...
(function ($) {
"use strict";
//jQuery객체 안에 하나의 객체선언*(jgrid)
$.jgrid = $.jgrid || {};
//jgrid의 맴버들(맴버함수, 맴버변수 추가)
//version, htmlDecode, htmlEncode(HTML내용을 escape character로..)
//format
$.extend($.jgrid,{
	version : "4.3.2",
	...
	//expr이 function이면.. expr함수에 obj파라메터를 태운 결과를 리턴한다.
	//아니면??  obj객체에서 expr에 해당하는객체가 있으면 그걸 리턴하고
	//아니면?? expr이 string이면? . 으로 split한 다음에 .....
	//왜 사용하는지 모르겠음..
	...
	// jLinq 라는 스크립트 라이브러리의 일부를 발췌....
	// Array로 구성된 데이터셋 형태의 객체를 클라이언트에서 핸들링하기 위한 용도임..
	// Usage : jgrid.from(어레이객체).filter(...).sort(...).select();
...
// jqGrid 플러그인함수( 실제 그리드 랜더링 수행)
// pin = parameter in ??
$.fn.jqGrid = function( pin ) {
	// pin이 string이면 해당 string의 함수를 찾아서 실행시킨다.
	if (typeof pin == 'string') {
		...
		//첫번째 이하의 아규먼트들을 해당 함수의 인자로 넘겨준다. 
		var args = $.makeArray(arguments).slice(1);
